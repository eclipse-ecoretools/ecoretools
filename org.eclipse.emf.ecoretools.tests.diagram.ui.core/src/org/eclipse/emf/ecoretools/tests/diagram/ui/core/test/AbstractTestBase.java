...
package org.eclipse.emf.ecoretools.tests.diagram.ui.core.test;
...
import org.eclipse.emf.ecoretools.tests.diagram.ui.core.fixture.IPresentationTestFixture;
 ...
 * @author choang
 * 
 * The abstract base class should be used by any tests that we write for the
 * shapes team It provides a framework for which the tests will run.
 * 
 ...
 * <p>
 * a. createConnectorViews - which setups up the diagram with the common shapes
 * and connectors that will be used for your tests
 * <p>
 * b. setDefaultDiagramExt - sets the diagram extension type which will be used
 * for the tests. Based on the ext given the diagram manager will use that as a
 * hint to determine which DiagramEditor class to use to manuipulate the
 * diagram.
 * <p>
 * c. public static suite() - to return the Test that will be our Action Menus
 * will run. Note this is not defined as an abstract method here because it
 * needs to be a static method in your test class. d. setTestFixtureLogic() -
 * which sets the
 * <code>org.eclipse.gmf.tests.runtime.diagram.ui.util.IPresentataionTestFixtureLogic</code>
 * class that will be responsible for creating the fixture(i.e test data) for
 * this test.
 * <p>
 * 2. Add your tests methods You need to name your tests method like test*. The
 * Junit framework will run all the methods that start with test*. For each test
 * in you class the Junit framework will first run the setup(), then your
 * testName1() method and the tearDown().
 * 
 * 
 */
public abstract class AbstractTestBase extends TestCase {
	 ...
	 * 
	 * Verbose mode can also be enabled using the {@link #SYSPROP_VERBOSE}
	 * system property.
	 * 
	 * @param enabled
	 *            boolean flag
	 ...
	 * 
	 * Sets the fixture logic for the tests. A fixture is the set of "data" that
	 * the test will run against Typically many tests will use the same fixture.
	 * 
	 */
	...
	protected DiagramState getDiagramState() {
		...
		try {
			return (DiagramState) TransactionUtil.getEditingDomain(getDiagram()).runExclusive(new RunnableWithResult.Impl() {
	 ...
	 * <code>ITestCommandCallBack</code>, which has the logic to verify that
	 * the command executed successful.
	 * <p>
	 ...
	 * <code>ITestCommandCallBack</code>, which has the logic to verify that
	 * the command executed successful. This method will test if the action
	 * implements the Disposable interface from GEF If it does it will call the
	 * dispose() method on the action. Callers should not call it themselves
	 * 
	 * @throws <AssertFailError>
	 *             if the command did not run successfully
	 * 
	 ...
	 * <code>ITestCommandCallBack</code>, which has the logic to verify that
	 * the command executed successful. This method will test if the action
	 * implements the IDisposableAction interface from common.ui If it does it
	 * will first set active the diagrameditorpart of the diagram and then call
	 * the init() method before running the action. At the end, it will call the
	 * dispose() method on the action. Callers should not call these two methods
	...
	protected void clearDiagram() {
		...
		testAction(GlobalActionManager.getInstance().createActionHandler(getWorkbenchPage(), GlobalActionId.DELETE), null);
	}
